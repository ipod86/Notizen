...
# Hinzugefügt: "requests" für den Webhook-Versand
...
def background_check():
    ...
    while True:
        try:
                        ...
                        for n in nodes:
                            ...
                            if r:
                                try:
                                    ...
                                    # Wenn Termin fällig und noch nicht gesendet
                                    if r_dt <= now and n['id'] not in sent_reminders:
                                        ...
                                        if url:
                                            # URL sicher machen für GET Parameter
                                            ...
                                            if method == 'GET':
                                                ...
                                                # POST: Ersetze Platzhalter im JSON Payload
                                                post_data = payload.replace('{{TITLE}}', title).replace('{{TIME}}', r)
                                                try:
                                                    # Versuche als reines JSON zu senden
                                                    json_data = json.loads(post_data)
                                                    requests.post(final_url, json=json_data, timeout=5)
                                                except:
                                                    # Fallback auf reinen Text, wenn das JSON fehlerhaft ist
                                                    requests.post(final_url, data=post_data.encode('utf-8'), headers={'Content-Type': 'application/json'}, timeout=5)
                                        
                                        sent_reminders.add(n['id'])
            ...
            pass # Stilles Scheitern, damit der Thread nicht stirbt
        ...
        <div class="modal" style="max-width: 500px;">
            ...
            <div style="text-align:left; margin-bottom: 15px;">
                ...
                <select id="webhook-method" onchange="toggleWebhookPayload()" style="width:100%; padding:10px; margin-bottom:10px; background:rgba(255,255,255,0.05); color:inherit; border:1px solid var(--border-color); border-radius:4px;">
                    <option value="GET">GET (Einfache URL, z.B. ntfy.sh)</option>
                    <option value="POST">POST (Mit JSON Payload, z.B. Discord)</option>
                </select>
...
function updateMenuUI() {
    ...
    if(whToggleText) {
        whToggleText.innerText = fullData.settings.webhook_enabled ? '🔔 Webhook (Aktiviert)' : '🔕 Webhook (Deaktiviert)';
    }
}
