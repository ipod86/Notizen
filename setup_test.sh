...
import json, os, uuid, tarfile, io, shutil, time, base64, requests, urllib.parse, threading
                            ...
                            if r:
                                try:
                                    ...
                                    if r_dt <= now and sent_key not in sent_reminders:
                                        ...
                                        method = settings.get('webhook_method', 'GET')
                                        payload = settings.get('webhook_payload', '')
                                    ...
                                    print(f"[WEBHOOK] ⚠️ Datum-Parse Fehler bei '{n.get('title')}': {e}", flush=True)
                            
                            if n.get('children'):
                                check_recursive(n['children'])
                    
                    check_recursive(data.get('content', []))
        except Exception as e:
            pass 
        time.sleep(30)
...
def import_backup():
    ...
    try:
        with tarfile.open(fileobj=file, mode='r:*') as tar:
            if not any(n == 'data.json' or n.endswith('/data.json') for n in tar.getnames()):
...
if not os.path.exists(DATA_FILE): exit()
...
def extract(nodes):
    for node in nodes:
        ...
        for fname in os.listdir(UPLOAD_FOLDER):
            if fname in text: used.add(fname)
            if fname.startswith('sketch_') and fname.endswith('.png'):
                sid = fname.replace('sketch_', '').replace('.png', '')
                if f"[sketch:{sid}]" in text:
                    used.add(fname)
                    used.add(f"sketch_{sid}.json")
        if 'children' in node: extract(node['children'])
extract(data.get('content', []))
for fname in os.listdir(UPLOAD_FOLDER):
...
.tree-group { min-height: 10px; padding-left: 15px; }
.tree-item-container { margin: 2px 0; }
.tree-item { display: flex; align-items: center; padding: 5px; border-radius: 4px; cursor: pointer; }
.tree-item.active { background: rgba(var(--accent-rgb), 0.2); color: var(--accent); font-weight: bold; }
.search-wrapper { position: relative; margin-bottom: 10px; height: 40px; }
#search-input { width: 100%; height: 100%; background: rgba(255,255,255,0.05); border: 1px solid var(--border-color); color: inherit; padding: 0 35px 0 12px; border-radius: 5px; box-sizing: border-box; font-size: 0.95em; }
#search-input:focus { outline: none; border-color: var(--accent); }
#clear-search { position: absolute; right: 5px; top: 50%; transform: translateY(-50%); width: 30px; height: 30px; display: none; align-items: center; justify-content: center; cursor: pointer; opacity: 0.5; font-size: 1.1em; user-select: none; line-height: 1; }
#clear-search:hover { opacity: 1; color: var(--accent); }
.drag-handle { display: none; padding: 0 5px 0 0; cursor: grab; color: #888; font-weight: bold; user-select: none; font-size: 1.2em; }
body.edit-mode-active .drag-handle { display: inline-block; }
body.edit-mode-active .tree-item { cursor: default; border: 1px dashed transparent; }
body.edit-mode-active .tree-item:hover { border: 1px dashed rgba(255,255,255,0.1); }
body.edit-mode-active #toggle-all-btn { display: none; }
#sort-btn { display: none; }
body.edit-mode-active #sort-btn { display: inline-block; }
.tree-icon { padding: 0 8px; font-size: 1.1em; user-select: none; }
.tree-text { flex-grow: 1; padding: 2px 5px; overflow-wrap: anywhere; word-break: break-word; }
button { background: none; border: none; color: inherit; cursor: pointer; font-family: inherit; font-size: inherit; }
.add-sub-btn, .delete-btn { display: none; font-weight: bold; margin-left: 5px; }
body.edit-mode-active .add-sub-btn, body.edit-mode-active .delete-btn { display: inline-block; }
.add-sub-btn { color: var(--accent) !important; margin-left: auto; }
.delete-btn { color: #e74c3c !important; }
.toolbar { margin-bottom: 12px; display: flex; flex-wrap: wrap; gap: 4px; align-items: stretch; position: relative; z-index: 20; }
.tool-btn { display: flex; flex-direction: column; align-items: center; justify-content: center; min-width: 40px; min-height: 40px; border: 1px solid var(--border-color); border-radius: 4px; padding: 2px 4px; background: rgba(255,255,255,0.02); transition: background 0.2s; }
...
#text-color-input { width: 16px; height: 16px; padding: 0; border: 1px solid var(--border-color); background: none; cursor: pointer; border-radius: 3px; appearance: none; -webkit-appearance: none; display: block; margin: 0; flex-shrink: 0; }
#text-color-input::-webkit-color-swatch-wrapper { padding: 0; }
#text-color-input::-webkit-color-swatch { border: none; border-radius: 2px; }
#editor { flex-grow: 1; height: 100%; overflow-y: auto; padding: 60px 40px; box-sizing: border-box; position: relative; }
#display-area { line-height: 1.5; overflow-wrap: break-word; min-height: 1.5em; }
#display-area div { min-height: 1.2em; }
b, strong { font-weight: bold; }
input, textarea { width: 100%; background: rgba(255,255,255,0.05); color: inherit; border: 1px solid var(--border-color); padding: 12px; border-radius: 5px; box-sizing: border-box; margin-bottom: 10px; font-family: inherit; transition: border-color 0.2s; }
.code-container { position: relative; background: var(--code-bg); color: var(--code-text); padding: 15px; border-radius: 5px; margin: 10px 0; border: 1px solid var(--border-color); }
.copy-badge { position: absolute; top: 5px; right: 5px; background: var(--accent) !important; color: white; padding: 2px 8px !important; font-size: 0.7em; border-radius: 3px; opacity: 0.7; }
...
.modal { background: var(--sidebar-bg); padding: 25px; border-radius: 12px; border: 1px solid var(--border-color); text-align: center; max-width: 400px; }
...
#mobile-toggle-btn { position: fixed; left: var(--sidebar-width); top: 20px; z-index: 1010; background: var(--accent) !important; color: white; padding: 10px !important; border-radius: 0 5px 5px 0; transition: left 0.3s ease; }
body.sidebar-hidden #mobile-toggle-btn { left: 0; }
.header-actions { position: fixed; top: 15px; right: 20px; z-index: 1000; }
.dropdown-content { display: none; position: absolute; right: 0; top: 40px; background: var(--sidebar-bg); border: 1px solid var(--border-color); min-width: 220px; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }
...
#mention-dropdown { display: none; position: absolute; top: 65px; left: 0; width: 100%; max-width: 400px; background: var(--sidebar-bg); border: 1px solid var(--accent); border-radius: 8px; max-height: 250px; overflow-y: auto; z-index: 1000; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
...
.note-link { color: var(--accent); text-decoration: none; font-weight: bold; padding: 2px 6px; background: rgba(var(--accent-rgb), 0.1); border-radius: 4px; border: 1px solid rgba(var(--accent-rgb), 0.3); transition: all 0.2s; cursor: pointer; display: inline-block; margin: 0 2px; max-width: 100%; box-sizing: border-box; overflow-wrap: anywhere; word-break: break-all; white-space: normal; }
.note-link:hover { background: var(--accent); color: white; }
.dead-link { color: #888; text-decoration: none; padding: 2px 6px; background: rgba(255,255,255,0.05); border-radius: 4px; border: 1px solid var(--border-color); display: inline-block; margin: 0 2px; cursor: not-allowed; }
...
<head>
    ...
    <style> body { display: flex; justify-content: center; align-items: center; } .login-box { background: var(--sidebar-bg); padding: 30px; border-radius: 8px; border: 1px solid var(--border-color); text-align: center; width: 300px; box-shadow: 0 5px 20px rgba(0,0,0,0.2); } </style>
</head>
    ...
    lockInterval = setInterval(async () => {
        try {
            ...
            if (data.status === 'lost') {
                ...
                lockInterval = null;
                currentlyLocked = false;
                showModal("Sperre verloren!", "Jemand anderes hat die Bearbeitung erzwungen.", [
                    {label: "Verstanden", class: "btn-cancel", action: () => { 
                        if (document.body.classList.contains('edit-mode-active')) deactivateTreeEdit();
                        else disableEdit(); 
                    }}
                ]);
            }
...
function toggleWebhookModal() {
    ...
    document.getElementById('webhook-method').value = fullData.settings.webhook_method || 'GET';
    document.getElementById('webhook-url').value = fullData.settings.webhook_url || '';
    document.getElementById('webhook-payload').value = fullData.settings.webhook_payload || '';
...
async function saveWebhook() {
    fullData.settings.webhook_enabled = document.getElementById('webhook-enabled').checked;
    fullData.settings.webhook_method = document.getElementById('webhook-method').value;
    fullData.settings.webhook_url = document.getElementById('webhook-url').value;
    fullData.settings.webhook_payload = document.getElementById('webhook-payload').value;
    await saveToServer();
...
function updateCurrentNode() {
    ...
    if(n) n.text = document.getElementById('node-text').value; 
}
...
function cleanDataArray(arr) {
    ...
    return arr.filter(item => item !== null && item !== undefined).map(item => ({
        ...item, children: cleanDataArray(item.children)
    }));
}
...
async function checkAndReloadData() {
    if (document.getElementById('edit-mode').style.display === 'block' || document.body.classList.contains('edit-mode-active')) return;
    try {
        const res = await fetch('/api/notes?_t=' + Date.now());
        if (!res.ok) return;
        const data = await res.json();
...
async function toggleEditMode() { 
    if (!document.body.classList.contains('edit-mode-active')) {
        ...
        if (!locked) {
            showModal("System gesperrt", "Auf anderem Gerät bearbeitet.\n\nSperre erzwingen?", [
                { label: "Ja", class: "btn-discard", action: async () => { await acquireLock(true); activateTreeEdit(); }},
...
function renderMarkdown(text) { 
    ...
    html = html.replace(/\[note:([a-zA-Z0-9]+)\|([^\]]+)\]/g, (match, id, title) => {
        if (findNode(fullData.content, id)) return '<a href="#" onclick="tryNavigation(\'' + id + '\'); return false;" class="note-link">@ ' + title + '</a>'; 
        else return '<span class="dead-link" title="Notiz wurde gelöscht">@ <del>' + title + '</del></span>'; 
    });
...
function deleteItem(id) { 
    showModal("Löschen", "Sicher?", [ 
        { label: "Löschen", class: "btn-discard", action: () => { removeFromArr(fullData.content, id); if (activeId === id) activeId = null; renderTree(); saveToServer(); } }, 
        { label: "Abbruch", class: "btn-cancel", action: () => {} } 
    ]); 
}
...
async function enableEdit() { 
    ...
    if (!locked) {
        showModal("System gesperrt", "Auf anderem Gerät bearbeitet.\n\nSperre erzwingen?", [
            { label: "Ja", class: "btn-discard", action: async () => { await acquireLock(true); showEditArea(); }},
            { label: "Abbrechen", class: "btn-cancel", action: () => {} }
        ]);
        return;
    }
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Escape') {
        ...
        else if (document.getElementById('sketch-modal').style.display === 'flex') document.getElementById('sketch-modal').style.display = 'none';
