...
.add-sub-btn:hover { color: var(--accent); opacity: 1; }
...
details.spoiler .spoiler-content {
    padding: 15px;
    filter: var(--spoiler-blur);
    transition: filter 0.3s;
}

details.spoiler[open] .spoiler-content {
    filter: blur(0);
}
...
.menu-row:hover { background-color: var(--hover-bg); }
...
.mention-path { font-size: 0.75em; color: #888; margin-top: 2px; }
...
.sketch-btn.active { border-color: var(--accent); background: rgba(var(--accent-rgb), 0.1); }
#canvas-wrapper { border: 1px solid var(--border-color); background: #fff; overflow: hidden; touch-action: none; }
#sketch-canvas { width: 100%; height: auto; display: block; cursor: crosshair; }
